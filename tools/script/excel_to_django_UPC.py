...
csv_file_path = "/home/Vieri/project_dir/oee_impc/tools/kd_upload.csv"
...
for index, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    dl_ = row[col_dl]
    the_form = ProductionForm(user_group_id=user_id, start_time=start_,
                                end_time=end_, shift=shift, machine_id=mesin_, no_kk=kk_, dl_avail=dl_)
    the_form.save()
    # rm details
    output = row[col_fg] + row[col_reject] + row[col_waste]
    fresh = output * 0.771 # Proporsi produksi scrap
    scrap_ = output - fresh
    fresh_ = fresh
