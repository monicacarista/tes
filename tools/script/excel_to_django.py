...
csv_file_path = "/home/Vieri/project_dir/oee_impc/tools/tobeuploaded-PC.csv"
...
convert_dict = {col_kk: str, col_date: str, col_machine: int, col_dl: float, 
                col_scrap_usage: float, col_fg: float, col_hold: float, col_berat_unit: float,
...
fields = ["kk", "tanggal", "mesin", "dl", "scrap_usage", "fg", "hold", "berat_unit",
...
for index, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    the_form.save()
    # rm details
    rm = row[col_fg] + row[col_hold] + row[col_reject] + \
        row[col_trimming] + row[col_waste]
