...
csv_file_path = "/home/Vieri/project_dir/oee_impc/tools/pipe-tbu.csv"
...
convert_dict = {col_kk: str, col_kk_cleaning: bool, col_kk_trial: bool, col_date: str, col_machine: int, col_dl: float, 
                col_rm: float, col_scrap_usage: float, col_fg: float, col_hold: float, col_berat_unit: float,
...
fields = ["kk", "rm", "cleaning", "trial", "tanggal", "mesin", "dl", "scrap_usage", "fg", "hold", "berat_unit",
...
for kk in kk_list:
    ...
    is_trial = df[df[col_kk] == kk][col_kk_trial]
    is_trial = is_trial.iloc[0]
    try:
        entry = KK(no_kk=kk, cleaning=is_cleaning, trial=is_trial ,creation_date=tanggal, user_group_id=user_id)
        entry.save()
    except:
        pass
